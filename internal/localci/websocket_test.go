...
func TestStatusWebSocketSendsPayload(t *testing.T) {
	t.Parallel()
	...
	mux.HandleFunc("/ws/status", server.handleStatusWebSocket)
	...
	wsURL := "ws://" + listener.Addr().String() + "/ws/status?repo=" + url.QueryEscape(repoDir) + "&commit=" + url.QueryEscape(commit)
	...
	if !strings.Contains(string(data), `"name":"localci:test"`) {
		t.Fatalf("payload missing task: %s", string(data))
	}
}

...
func TestStatusReaderReadCommit(t *testing.T) {
	...
	taskA := newTaskRecord(paths, req, Task{Name: "localci:build"}, run.StartedAt)
	...
	taskB := newTaskRecord(paths, req, Task{Name: "localci:test"}, run.StartedAt)
	...
	run.TaskResults = []TaskRecord{taskA, taskB}
	run.RefreshSummary()
	...
	if status.Run.Summary.Total != 2 || status.Run.Summary.Failed != 1 {

...
Run an ad hoc check directly in the current terminal. Use `run` when you want daemon-managed queueing. `invoke` is intended to be a substitute for calling all your static analysis and test scripts ad hoc: use `localci invoke --no-clone` to just “test everything right here right now.”

## UIs

### `localci web [--repo dir] [--commit ref] [--task task] [--attempt n] [--artifact artifact] [--no-clone]`

Open the web UI, which shows comprehensive information including log files, live status, downloadable binaries, and HTML output of tasks.
